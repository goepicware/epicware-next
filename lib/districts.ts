...
export const DISTRICTS: DistrictData[] = [
  ...
  },
  {
    ...
    whyMatter:
      "Novena's concentration of healthcare providers creates one of Singapore's most review-sensitive local search environments. An inaccurate or unfair negative review on a clinic's Google profile can deter dozens of prospective patients who cannot independently verify the claim. Google's own review removal process is opaque and slow to navigate for individual businesses. Epicware's bad review removal service — $200 per review, refunded if not removed within 3 months — is particularly high-value for Novena clinics and specialist practices that need their online reputation to accurately reflect the quality of care they provide, not the agenda of a disgruntled or fraudulent reviewer.",
